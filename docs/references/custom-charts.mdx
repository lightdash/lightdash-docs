...
Custom charts are available in Beta for those that want additional customisation and an extended library of charts. 
...
Custom visualisations in Lightdash are a Beta feature which means we have limited documentation and support. We may also change the way these options work in the future.

:::

## Enabling Custom Charts

Custom charts need to be manually enabled in your project. 

- **Cloud Users**: Contact support for help enabling this feature.
- **Self-Hosted Users**: Set the following environment variable:

```sh
CUSTOM_VISUALIZATIONS_ENABLED=true
```
...
Bubble plots build on top of standard scatter plot visualisations, by allowing you to adjust the size of a given point based on the output of a field. Here's one looking at some Healthcare data.
