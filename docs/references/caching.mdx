import CachedFilter from './assets/cached-filter.png';
import DashboardCacheTime from './assets/dashboard-cache-time.png';
...
1. **[Cached Filter values](#filter-value-caching)** - enabled for everyone without requiring any configuration.
...
This type of caching works without any configuration. You'll see a message in your filter values that tells you when the cached filter values were loaded from (usually within the last day). If you want to refresh the filter values, you can click on that message and the values will be refreshed. 
...
Any changes to the chart query or dashboard queries (user attributes, filters, limit, date zoom) will trigger new queries to the warehouse and create a separate cache entry.
...
- All table queries are currently cached, even when using the query builder before a chart is saved. In the future Lightdash will [likely disable caching in the query builder](https://github.com/lightdash/lightdash/issues/12697) since most of these queries are one-off explorations and don't benefit from caching.
- Saved Charts are cached based on the last refresh in any context (edit mode, view mode, dashboard refresh, etc.)
- Dashboard tiles will use the cache from Saved Charts they reference.
- Scheduled Deliveries generate and use cached results. 
