...
To recap, you can set the default visibility of metrics in the catalog by adding the following to your `lightdash.config.yml` file:
...
This will make the `event_count` metric visible in the catalog, even though the events model is hidden.
...
To set this up, you need to add the following to your `lightdash.config.yml` file:
