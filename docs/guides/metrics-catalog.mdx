...
import ExploringAMetric from "./assets/exploring-a-metric.webp";

# Metrics Catalog

:::info
🏗️ The Metrics Catalog is currently in beta. We're actively testing and improving it—some of the functionality might be subject to change.
:::

The Metrics Catalog simplifies data exploration, making it easy for business users to focus on metrics without dealing with complex tables or time dimensions.
...
<img
  src={ExploringAMetric}
  width="1240"
  height="580"
  style={{ display: "block", margin: "0 auto 20px auto" }}
/>

1. **Plot over time**: View your selected metric plotted over a time dimension. You can adjust this dimension or set a default time dimension in your `.yml` to simplify the process for users. [See how](#configuring-default-time-settings-in-yml)
2. **Filter by date**: Use the date picker with convenient presets for quick selection.
...
4. **Compare to:**
   ...
   - **'Another metric':** Compare to any other metric in the project **that has a default time dimension specified in the `.yml`**. [See how](#configuring-default-time-settings-in-yml)
5. **Segment-by**: Break down the metric by any non-time dimension in your dataset.

## Configuring default time settings in .yml

To pre-select a time dimension in the explore view, you need to define default dimensions in your Lightdash `.yml` files.
